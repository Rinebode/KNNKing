...
int main(void) {
    ...
    while (letter <= 'Z') {
        ...
        switch (choice) {
            case 0:
                if (grid[y_pos][x_pos + 1] == '.' && x_pos + 1 < 10) {
                    grid[y_pos][x_pos + 1] = letter;
                    x_pos += 1;
                    advanced = true;
                } else {
            ...
            case 1:
                if (grid[y_pos][x_pos - 1] == '.' && x_pos - 1 >= 0) {
                    grid[y_pos][x_pos - 1] = letter;
                    x_pos -= 1;
                    advanced = true;
                } else {
            ...
            case 2:
                if (grid[y_pos - 1][x_pos] == '.' && y_pos + 1 <= 0) {
                    grid[y_pos - 1][x_pos] = letter;
                    y_pos -= 1;
                    advanced = true;
                } else {
            ...
            case 3:
                if (grid[y_pos + 1][x_pos] == '.' && y_pos + 1 < 10) {  // Note: fixed index typo from your original
                    grid[y_pos + 1][x_pos] = letter;
                    y_pos += 1;
                    advanced = true;
                } else {
